...
class Miscs(commands.Cog):
    ...
    async def _display_member_info(self, interaction: discord.Interaction, member: discord.Member = None):
        try:
            ...
            embed.add_field(name="Name", value=member_name)
            embed.add_field(name="ID", value=member_id)
            embed.add_field(name="Top Role", value=member_top_role)
            embed.add_field(name="Joined At", value=f"<t:{member_joined_at}:F>\n(<t:{member_joined_at}:R>)")
            embed.add_field(name="Created At", value=f"<t:{member_created_at}:F>\n(<t:{member_created_at}:R>)")
            embed.add_field(name="Nitro?*", value=member_has_nitro)
            ...
            embed.add_field(name="Top Role Perms", value="\n".join(lst))
